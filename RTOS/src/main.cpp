...
static bool isMeasurementState() {
  return measurementActive &&
         (currentState == STATE_MEASURE ||
          currentState == STATE_MEAS_CAL ||
          currentState == STATE_CAL2);
}
...
void mainControlTask(void *pvParameters){
  ...
  for(;;){
    ...
    if (commandReady && strcmp(command, "next") == 0) {
      commandReady = false;
      ...
      s.print(F("Preskakuji do stavu: "));
      s.println((int)currentState);
      continue;
    }

    if (currentState != lastPromptedState) {
      if (currentState == STATE_INIT) s.println(F("Pripraveno. Posli 'init'."));
      if (currentState == STATE_CAL) s.println(F("Kalibrace. Posli 'cal1' pro otevreni serva."));
      if (currentState == STATE_MEAS_CAL) s.println(F("Kalibracni mereni. Posli 'meas_cal' pro zacit mereni teploty a pH."));
      if (currentState == STATE_CAL2) s.println(F("Kalibrace probiha. Posli 'cal2' pro ukonceni a zavreni."));
    ...
    switch(currentState){
      case STATE_INIT: {
        if (takeCommand("init")) {
          ...
          pinMode(CAL_SERVO_PIN, OUTPUT);
          digitalWrite(EN_PIN, HIGH);

          pinMode(FLOW_PIN, INPUT);
          attachInterrupt(digitalPinToInterrupt(FLOW_PIN), pulseCounter, RISING);
          
          s.println(F("Kalibruji vychozi pozici serva (0 stupnu)..."));

          calServo.setPeriodHertz(50);
          calServo.attach(CAL_SERVO_PIN, 500, 2400);

          for (int angle = 0; angle <= 180; angle += 2) {
            calServo.write(angle);
      ...
      case STATE_CAL: {
        // Čeká se POUZE na příkaz cal1
        if (takeCommand("cal1")) {
          s.println(F("Oteviram servo na 0 stupnu..."));
          
          // Plynulé otevření serva ze 180 do 0 stupňů
          for (int angle = 180; angle >= 0; angle -= 2) {
            calServo.write(angle);
            vTaskDelay(pdMS_TO_TICKS(25));
          }
          
          // Potvrzení koncové otevřené polohy pro stabilizaci
          for (int i = 0; i < 20; i++) {
            calServo.write(0);
            vTaskDelay(pdMS_TO_TICKS(25));
          }
          
          // Přechod do samostatného stavu kalibracniho mereni
          currentState = STATE_MEAS_CAL;
        }
        break;
      }
      ...
      case STATE_MEAS_CAL: {
        if (takeCommand("meas_cal")) {
          s.println(F("Zacinam kalibracni mereni teploty a pH..."));
          measurementActive = true;
          currentState = STATE_CAL2;
        }
        break;
      ...
      case STATE_CAL2: {
        // Zde už běží měření a čekáme POUZE na cal2
        if (takeCommand("cal2")) {
          s.println(F("Ukoncuji kalibraci, zaviram servo na 180 stupnu..."));
          
          // Vypnutí měření ihned, než se servo začne zavírat
          measurementActive = false;
          s.printf("Konec kalibracniho mereni. Teplota: %.2f C, pH: %.2f\n", currentTemp, currentPH);
          
          // Plynulé zavření serva z 0 zpět na 180 stupňů
          for (int angle = 0; angle <= 180; angle += 2) {
            calServo.write(angle);
            vTaskDelay(pdMS_TO_TICKS(25));
          }
          
          // Potvrzení koncové zavřené polohy pro stabilizaci
          for (int i = 0; i < 20; i++) {
            calServo.write(180);
            vTaskDelay(pdMS_TO_TICKS(25));
          }
          
          // Přechod do dalšího stavu podle původního zadání
          currentState = STATE_MEASURE;
        }
        break;
      }
...
void pHTask(void *pvParameters) {
  ...
  for (;;) {
    if (isMeasurementState()) {
      ...
      float currentVoltage = averageMilliVolts / 1000.0f - PH_ADC_OFFSET_V;
